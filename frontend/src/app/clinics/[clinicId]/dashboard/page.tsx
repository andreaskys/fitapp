...
import { UserPlus, UserCircle2, ChevronRight, Loader2, Trash2, Users, CalendarCheck, Settings, LogOut, User } from "lucide-react";
...
export default function ClinicDashboard({ params }: { params: Promise<{ clinicId: string }> }) {
    ...
    useEffect(() => {
        Promise.all([
            apiFetch(`/api/patients?clinicId=${clinicId}`).then(res => res.json()),
            apiFetch(`/api/clinics/${clinicId}/metrics`).then(res => res.json()),
            apiFetch(`/api/clinics/${clinicId}`).then(res => res.json())
        ])
            .then(([patientsData, metricsData, clinicData]) => {
                setPatients(patientsData);
                setMetrics(metricsData);
                setClinicName(clinicData.name || "Clinic Workspace");
                setIsLoading(false);
            })
            .catch(err => {
                console.error("Error fetching dashboard data:", err);
                setClinicName("Clinic Workspace"); // Fallback on error
                setIsLoading(false);
            });
    }, [clinicId]);
    ...
    const handleDeletePatient = async (e: React.MouseEvent, id: number, name: string) => {
        e.preventDefault();
        if (!window.confirm(`Permanently delete ${name} and all their clinical history?`)) return;
        try {
    ...
    return (
        <div className="p-8 md:p-12 max-w-7xl mx-auto min-h-screen bg-[#fbfbfd]">
            ...
            {/* NEW: Top Navigation & User Dropdown */}
            <div className="flex justify-end w-full mb-8 relative z-50">
                ...
                >
                    <div
                        className="w-8 h-8 bg-zinc-50 rounded-full flex items-center justify-center border border-zinc-100">
            ...
            >
                <div className="flex items-center justify-between mb-10 border-b border-zinc-100 pb-4">
                        ...
                        <div key={patient.id} className="relative block group">
                            <Link href={`/clinics/${clinicId}/patients/${patient.id}`} className="block">
                                <motion.div whileHover={{x: 5}}
                                            className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 rounded-2xl hover:bg-zinc-50/80 transition-all duration-300 border border-transparent hover:border-zinc-100 cursor-pointer pr-16">
                                    <div className="flex-1">
                                        <h3 className="text-xl font-bold tracking-tight text-black group-hover:text-blue-600 transition-colors">
                                            {patient.name}
                                        </h3>
                                        <div
                                            className="flex items-center gap-4 text-xs font-medium text-zinc-400 uppercase tracking-widest mt-1">
                                            Source: {patient.referralSource || "Internal"}
                                            {patient.instagramUser &&
                                                <span className="text-blue-500">{patient.instagramUser}</span>}
                                        </div>
                                    </div>
                                    <div className="mt-4 sm:mt-0 flex justify-end gap-3 items-center">
                                        <span
                                            className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full">Active File</span>
                                        <div
                                            className="w-10 h-10 rounded-full bg-white flex items-center justify-center border border-zinc-100 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
